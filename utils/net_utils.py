...
def Entropy(args, input_, open_flag=False):
    bs = input_.size(0)
    epsilon = 1e-5
    if open_flag == True:
        entropy = -1/(args.class_num) * torch.log(input_ + epsilon)
    else:
...
def compute_h_score(args, class_list, gt_label_all, pred_cls_all, open_flag=True, open_thresh=0.5, pred_unc_all=None):
    ...
    if open_flag:
        ...
        if pred_unc_all is None:
            ...
            pred_unc_all = Entropy(args, pred_cls_all)/np.log(cls_num)# [N]
...
def inference(args, model, dataloader, apply_softmax=True):
    feature_all = []
    pred_all = []
    label_all = []
    model.eval()
...
def ring_loss_unknown_only(y_pred, y_true):
    """
    Computes the ring loss for unknown samples only.

    Args:
        y_pred (Tensor): (batch_size, embedding_dim) - Predicted feature vectors.
        y_true (Tensor): (batch_size, 2) - One-hot labels where:
                         - y_true[:, 1] = 1 for unknown classes, 0 otherwise.

    Returns:
        Tensor: Scalar loss value.
    """
    pred_norm = torch.norm(y_pred, p=2, dim=1)

    loss_unknown = y_true[:, 1] * pred_norm  # Penalizes large magnitudes

    return torch.mean(loss_unknown ** 2)

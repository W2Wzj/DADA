...
class VGGBase(nn.Module):
    def __init__(self, backbone_arch):
        ...
        model_vgg = vgg_dict[backbone_arch](pretrained=True)
...
class ResBase(nn.Module):
    def __init__(self, backbone_arch):
        ...
        model_resnet = res_dict[backbone_arch](pretrained=True)
...
class Embedding(nn.Module):
    def __init__(self, feature_dim, embed_dim=256, type="ori"):
        ...
        self.bottleneck = nn.Linear(feature_dim, embed_dim)  # one linear layer
        self.bottleneck.apply(init_weights)  # apply initial weight to bottleneck
        self.type = type

...
def train(args, model, dataloader, criterion, optimizer, epoch_idx=0.0):
    ...
    for imgs_train, _, imgs_label, _ in tqdm(dataloader, ncols=60):
        ...
        imgs_label = imgs_label.to(args.device)
...
def main(args):
    ...
    if args.checkpoint is not None and os.path.isfile(args.checkpoint):
        ...
        # save_dir = os.path.join(this_dir, "checkpoints_source_ablation", "source_{}".format(args.s_idx),args.dataset, "target_num{}".format(args.target_private_class_num), "source_num{}".format(args.source_private_class_num))
        save_dir = os.path.join(this_dir, "checkpoints_source_ablation", args.dataset, "source_{}".format(args.s_idx), "#target{}".format(args.target_private_class_num),"#source{}".format(args.source_private_class_num))
    ...
    optimizer = op_copy(optimizer)  # add lr01

    source_data_list = open(os.path.join(args.source_data_dir, "image_unida_list.txt"), "r").readlines()
    source_dataset = SFDADataset(args, args.source_data_dir, source_data_list, d_type="source", preload_flg=True)
    source_dataloader = DataLoader(source_dataset, batch_size=args.batch_size, shuffle=True,
                                   num_workers=args.num_workers, drop_last=True)   # batch
    ...
    for idx in range(len(args.target_domain_dir_list)):
        ...
        target_dataset = SFDADataset(args, target_data_dir, target_data_list, d_type="target", preload_flg=False)
